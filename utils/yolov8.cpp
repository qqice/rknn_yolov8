...
int Yolov8::Init(rknn_context *ctx_in, bool copy_weight) {
  ...
  if (model == nullptr) {
      spdlog::error("Load model failed");
    return -1;
  }
  if (copy_weight) {
      spdlog::info("rknn_dup_context() is called");
    // 复用模型参数
    ret = rknn_dup_context(ctx_in, &ctx_);
    if (ret != RKNN_SUCC) {
        spdlog::error("rknn_dup_context failed! error code = {}", ret);
      return -1;
    }
  } else {
      spdlog::info("rknn_init() is called");
    ret = rknn_init(&ctx_, model, model_len, 0, NULL);
    free(model);
    if (ret != RKNN_SUCC) {
        spdlog::error("rknn_init failed! error code = {}", ret);
      return -1;
    }
  }
  ...
  if (ret < 0) {
      spdlog::error("rknn_set_core_mask failed! error code = {}", ret);
    return -1;
  }
  ...
  if (ret < 0) {
    ...
    spdlog::info("sdk version: {} driver version: {}", version.api_version,
                     version.drv_version);
  ...
  if (ret != RKNN_SUCC) {
      spdlog::error("rknn_query failed! error code = {}", ret);
    return -1;
  }
    spdlog::info("model input num: {}, and output num: {}", io_num.n_input,
                     io_num.n_output);
  ...
  for (int i = 0; i < io_num.n_input; i++) {
    ...
    if (ret != RKNN_SUCC) {
        spdlog::error("input rknn_query failed! error code = {}", ret);
      return -1;
    }
  ...
  // Get Model Output Info
    spdlog::info("output tensors:");
  rknn_tensor_attr output_attrs[io_num.n_output];
  memset(output_attrs, 0, sizeof(output_attrs));
  // rk官方的segment模型有13个输出
  if (io_num.n_output == 13) {
      spdlog::info("this is a segment model");
  ...
  for (int i = 0; i < io_num.n_output; i++) {
    ...
    ret = rknn_query(ctx_, RKNN_QUERY_OUTPUT_ATTR, &(output_attrs[i]),
                     sizeof(rknn_tensor_attr));
    if (ret != RKNN_SUCC) {
        spdlog::error("output rknn_query fail! error code = {}", ret);
      return -1;
    }
...
int Yolov8::Inference(void *image_buf, object_detect_result_list *od_results,
  ...
  int ret = rknn_inputs_set(app_ctx_.rknn_ctx, app_ctx_.io_num.n_input,
                            inputs_.get());
  if (ret < 0) {
      spdlog::error("rknn_input_set failed! error code = {}", ret);
    return -1;
  }
  ret = rknn_run(app_ctx_.rknn_ctx, nullptr);
  if (ret != RKNN_SUCC) {
      spdlog::error("rknn_run failed, error code = {}", ret);
    return -1;
  }
  ...
  if (ret != RKNN_SUCC) {
      spdlog::error("rknn_outputs_get failed, error code = {}", ret);
    return -1;
  }
